/**
 * Exercise: 5-20 - Expand dcl to handle declarations with function argument type, 
 * qualifiers like const, and so on.
 * 
 * Please note that the snippet in The C Answer Book is not complete 
 **/
...
extern char token[];                   // last token string
extern char name[];                    // identifier name
extern char datatype[];                // data type  = chart, int, etc.
extern char out[]; 
extern int prevtoken;
...
void paramdcl(void) {
    ...
    if (tokentype != ')')
        errmsg("missing ) in parameter declaraction\n");
}
...
int typequal(void) {
    ...
    if (bsearch(&pt, types, sizeof(typeq)/sizeof(char *),
          sizeof(char *), compare) == NULL) 
        return NO;
    else
        return YES;
